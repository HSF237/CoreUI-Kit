...
export default function CircularProgressStats(){
  return <section className="grid w-full max-w-3xl gap-3 rounded-[28px] border border-white/10 bg-[#07101c] p-5 shadow-2xl shadow-black/35 sm:grid-cols-3 sm:p-6">
    {stats.map(([label,value,percent,color])=><article key={label} className="rounded-2xl border border-white/10 bg-white/[0.025] p-4 text-center">
      <div className="mx-auto flex h-28 w-28 items-center justify-center rounded-full" style={{background:"conic-gradient("+color+" 0 "+percent+"%, rgba(255,255,255,.06) "+percent+"% 100%)"}}><div className="flex h-20 w-20 items-center justify-center rounded-full bg-[#09121f]"><span className="text-xl font-semibold text-white">{value}</span></div></div>
