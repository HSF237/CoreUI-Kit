...
export default function ProfileSettingsForm(){
  return <section className="w-full max-w-2xl rounded-[28px] border border-white/10 bg-[#07101c] p-5 shadow-2xl shadow-black/35 sm:p-6">
    ...
    <div className="mt-6 grid gap-4 sm:grid-cols-2">{[["First name","Maya"],["Last name","Kim"],["Role","Product Designer"],["Company","Northstar Labs"]].map(([label,value])=><label key={label}><span className="text-[11px] font-medium text-slate-500">{label}</span><input defaultValue={value} className="mt-2 h-11 w-full rounded-xl border border-white/10 bg-white/[0.025] px-3 text-sm text-slate-200 outline-none transition focus:border-cyan-300/30"/></label>)}</div>
    <label className="mt-4 block"><span className="text-[11px] font-medium text-slate-500">Bio</span><textarea defaultValue="Designing fast, thoughtful product experiences." className="mt-2 min-h-24 w-full resize-none rounded-xl border border-white/10 bg-white/[0.025] p-3 text-sm text-slate-200 outline-none focus:border-cyan-300/30"/></label>
