...
export default function RevenueMetricCard(){
  return <section className="w-full max-w-xl rounded-[28px] border border-white/10 bg-[#07101c] p-5 shadow-2xl shadow-black/35 sm:p-6">
    ...
    <div className="mt-8 flex h-36 items-end gap-2 rounded-2xl border border-white/10 bg-black/10 p-4">{points.map((p,i)=><div key={i} className="group relative flex h-full flex-1 items-end"><div className="w-full rounded-t-md bg-gradient-to-t from-cyan-400/15 to-cyan-300/70 transition group-hover:to-violet-300/80" style={{height:p+"%"}}/></div>)}</div>
